...
/**
 * Класс BaseExceptions является пользовательским исключением, производным от класса Exception. Он
 * используется для представления исключительных ситуаций в контексте базовых ошибок.
 */
public class BaseExceptions extends Exception {
  ...
  /**
   * Переопределение метода getMessage() из класса Exception. Возвращает сообщение об ошибке,
   * связанное с пользовательским исключением.
   *

...
public class ParserEngineTemplate {
    ...
    while (reader.hasNext()) {
      ...
      if (Nodes.TEMPLATE.equals(elementName) && readCode == XMLStreamConstants.START_ELEMENT) {
        processTemplate(reader, templateMap);
          ...
          && readCode == XMLStreamConstants.END_ELEMENT) {
        break;
      }
    }
    return templateMap;
  ...
  private static void processTemplate(
      XMLStreamReader reader, HashMap<String, Elements.Template> templateMap)
      throws XMLStreamException {
    ...
    if (templateMap.containsKey(template.getId())) {
      throw new RuntimeException(
          String.format("Шаблон с ID '%s' уже существует в хранилище", template.getId()));
    }

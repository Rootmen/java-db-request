...
public static Map<String, Elements.Parameter<?>> parseParametersNode(XMLStreamReader reader)
	...
	} else if (parserCode == XMLStreamConstants.END_ELEMENT
		&& Nodes.PARAMETERS.equals(localName)) {
		break;
	}
	}
	return parameters;
...
private static Elements.Parameter<?> processParameter(XMLStreamReader reader)
	throws XMLStreamException {
	...
	ParameterAbstract<?> parameterAbstract =
		(ParameterAbstract<?>)
			ParameterFactory.getParameter(parameterName, parameterType, defaultValue);
	...
	if (parserCode == XMLStreamConstants.START_ELEMENT && Nodes.WHEN.equals(localName)) {
		...
		throw new RuntimeException(
			String.format("When значения дублируются с value='%s'", whenValue));
		}
	...
	if (parserCode == XMLStreamConstants.START_ELEMENT && Nodes.OTHERWISE.equals(localName)) {
		...
		&& Nodes.PARAMETER.equals(localName)) {
		break;
	}
	}

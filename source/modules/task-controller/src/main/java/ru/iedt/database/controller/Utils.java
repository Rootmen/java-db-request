...
public static TaskDescription parseMessageFromRest(Message<JsonObject> payload) {
	Optional<IncomingRabbitMQMetadata> metadata =
		payload.getMetadata(IncomingRabbitMQMetadata.class);
	if (metadata.isEmpty()) {
...
public static TaskWebsocketDescription parseMessageFromWebsocket(Message<JsonObject> payload) {
	...
	return new TaskWebsocketDescription(
		UUID.fromString(
			mqMetadata.getHeader(Attributes.Websocket.USER_ID, String.class).orElse("")),
		...
		UUID.fromString(
			mqMetadata.getHeader(Attributes.Websocket.TASK_ID, String.class).orElse("")),

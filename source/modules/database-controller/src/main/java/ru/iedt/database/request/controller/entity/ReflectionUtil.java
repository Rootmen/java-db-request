...
public class ReflectionUtil {
  ...
  static Object getDataFromRow(
    ...
    if (createParameter.is_json_mapping()) {
      return getDataFromJson(row, clazz, createParameter, typeClass);
    }
    ...
    return type.apply(row, createParameter.value());
  }

  static ObjectMapper mapper = new ObjectMapper();

  @SuppressWarnings("unchecked")
  static Object getDataFromJson(
    ...
    try {
      if (clazz == ArrayList.class || clazz == LinkedList.class || clazz == List.class) {
        ...
        return mapper.readValue(row.getJson(createParameter.value()).toString(), type);
      }
      return mapper.readValue(row.getJson(createParameter.value()).toString(), clazz);
  ...
  static Object getListArrayFromRow(
    ...
    if (typeGeneric instanceof ParameterizedType) {
      Class<?> arrayClazz =
          (Class<?>) ((ParameterizedType) typeGeneric).getActualTypeArguments()[0];
      Object[] array =
          RowTypes.typeArrayFunctionHashMap.get(arrayClazz).apply(row, createParameter.value());

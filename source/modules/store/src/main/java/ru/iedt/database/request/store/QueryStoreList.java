...
*
* <p>Этот метод использует рефлексию для поиска классов и создания их экземпляров, отмеченных
* аннотацией @DefinitionStore, для извлечения и хранения метаданных.
*
* @return Список QueryStoreDefinition, содержащий метаданные классов, отмеченных
*     аннотацией @DefinitionStore.
* @throws RuntimeException В случае возникновения ошибок при создании экземпляров классов с
*     метаданными.
*/
public static ArrayList<QueryStoreDefinition> getStoresMetadata() {
	...
	ArrayList<Class<?>> classArrayList =
		new ArrayList<>(reflections.getTypesAnnotatedWith(DefinitionStore.class));

	System.out.printf("Найдено аннотаций: %-10s", classArrayList.size());
	ArrayList<QueryStoreDefinition> queryStores = new ArrayList<>();

	// Итерация по классам с аннотацией @DefinitionStore и создание их экземпляров в массив
	// queryStores
...
/**
* Вложенный статический класс QueryStore представляет собой структуру, которая хранит информацию
* о хранилище запросов.
*/
